...
const ResetPassword: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const validateToken = async () => {
      try {
        const res = await axios.get(`http://localhost:3000/api/v1/shelter/resetpassword/${token}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await axios.post(`http://localhost:3000/api/v1/shelter/resetpassword/${token}`, {
        email,
